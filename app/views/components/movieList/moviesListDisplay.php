<section id="display-list-movies">
    <div class="container d-grid justify-content-center">
        <div class="list-container">
            <div class="no-movies">
                <h4>No se han encontrado películas</h4>
            </div>
            <?php foreach ($movies as $movie): ?>
                <div class="movie">
                    <div class="movie-container">
                        <div>
                            <a href="/proyectodaw/movie/findOne/<?php echo $movie->id ?>">
                                <img class="image-fluid poster" src="<?php echo $movie->poster ?>" alt="...">
                            </a>
                        </div>
                        <div class="row">
                            <div class="col-10">
                                <h4 class="title-movie">
                                    <a href="/proyectodaw/movie/findOne/<?php echo $movie->id ?>">
                                        <?php echo $movie->title ?>
                                    </a>
                                </h4>
                                <p class="genres"><?php echo implode(", ", $movie->genres) ?></p>
                                <p class="release-date"><?php echo substr($movie->releaseDate, 0, 4) ?></p>
                                <i class="fa fa-star"></i><span class="px-1"><?php echo $movie->score?>/5</span>
                            </div>
                            <?php if(isset($logged) && $logged):?>

                            <?php
                                if(isset($favoriteMovies) && in_array($movie->id, $favoriteMovies)){
                                    $value = 'btn-danger';
                                }else{
                                    $value = 'btn-outline-secondary';
                                }
                                ?>
                            <div class="col-2">
                                <button type="button" class="btn <?php echo $value?>" onclick="likeButton(this, <?php echo $movie->id?>)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                         class="bi bi-heart" viewBox="0 0 16 16">
                                        <path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143q.09.083.176.171a3 3 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15"></path>
                                    </svg>
                                </button>
                            </div>
                            <?php endif?>
                            <p class="body"><?php echo $movie->body ?></p>
                        </div>
                    </div>
                    <hr>
                </div>
            <?php endforeach; ?>
        </div>
    </div>

</section>